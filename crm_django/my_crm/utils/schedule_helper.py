...
from ..models import MonthSchedule, DayType, SchedulePattern
...
class PatternDict(TypedDict, total=False):
    days_off_at_start: int
    cycle_length: int
    last_working_day: bool
    exceptions: list[tuple[int, str]]
    start_with_workday: bool


def return_pattern(schedule: MonthSchedule) -> PatternDict:

    pattern = schedule.pattern

    return {
        "days_off_at_start": pattern.days_off_at_start,
        "cycle_length": sum(pattern.pattern_after_start or []),
        "last_working_day": pattern.last_day_always_working,
        "exceptions": [],  # тут потом можно будет подставить реальные исключения из базы
        "start_with_workday": True,
    }
